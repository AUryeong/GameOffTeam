...
#pragma warning disable 0649
		...
		[SerializeField]
		private PointerEventListener showToggle;
...
#pragma warning restore 0649
		...
		public void Initialize( RuntimeHierarchy hierarchy )
		{
			...
			showToggle.PointerClick += (eventData) => ToggleShowState();
		}
		...
		{
			HierarchyDataTransform trData = Data as HierarchyDataTransform;
			if (trData == null)
				return;

			if (trData.Transform == null)
				return;

			if (trData.Transform.gameObject.name.Contains("HierarchyRoot"))
				return;

			trData.Transform.gameObject.SetActive(!trData.Transform.gameObject.activeSelf);
		}
