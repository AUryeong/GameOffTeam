...
public class LMJ_Utill : MonoSingleton<LMJ_Utill>
{
    ...
    public void GetIP(System.Action<string> func)
    {
        ...
        (msg) =>
        {
            ADN.LogError(string.Format("[{0}] {1}", "GetIP", msg));
        });
        */
    ...
    static public void SendLogMail(string title, string msg)
    {
        ...
        mail.From = new MailAddress("ADN.Log.recv@gmail.com");
        mail.To.Add("ADN.Log.recv@gmail.com");
        mail.Subject = title;
        mail.Body = msg;

        SmtpClient smtpServer = new SmtpClient("smtp.gmail.com");
        smtpServer.Port = 587;
        smtpServer.Credentials = new System.Net.NetworkCredential("ADN.Log.recv@gmail.com", "Eoqkrsksek!") as ICredentialsByHost;
        smtpServer.EnableSsl = true;
        ServicePointManager.ServerCertificateValidationCallback =
            delegate (object s, X509Certificate certificate, X509Chain chain, SslPolicyErrors sslPolicyErrors)
            { return true; };
        smtpServer.Send(mail);
        LMJ.Log("SendLogMail success : to ADN.Log.recv@gmail.com ");
    }
    ...
    //    //ADN.Log(string.Format("hitPixel {0}  {1}", localHit, hitPixel));
    ...
    //    if (textAsset == null)
    //    {
    //        ADN.LogError("Can`t load file ! " + path);
    //    }
    //    return textAsset;
    //}
    ...
    public static void DestroyParticles(GameObject from)
    {
        List<ParticleSystem> parts = GetComponentsRecursively<ParticleSystem>(from);
        CheckExcute.Subscribe(() =>
       {
           if (from == null)
           {
               LMJ.Log("DestroyParticles :: isDestroyed??");
               return true;
           }

           for (int n = 0; n < parts.Count; n++)
           {
               if (parts[n].isPlaying)
                   return false;
           }
    ...
    public static void AddOrderLayer(GameObject gameObject, int value)
    {
        return;
        List<Renderer> rs = LMJ_Utill.GetComponentsRecursively<Renderer>(gameObject);
        for (int i = 0; i < rs.Count; i++)
        {
            rs[i].sortingOrder += value;
        }
    }
    ...
    //    ADN.AnimatedButton[] aniBtns = btn.GetComponentsInChildren<ADN.AnimatedButton>();
